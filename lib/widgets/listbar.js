...
function Listbar(options) {
  ...
  this.disableKeysPrefix = options.disableKeysPrefix || false;
...
Listbar.prototype.setItems = function(commands) {
  ...
  if (!Array.isArray(commands)) {
    commands = Object.keys(commands).reduce(function(obj, key, i) {
      ...
      if (cmd.text === undefined) cmd.text = key;
      if (cmd.prefix === undefined) cmd.prefix = String(++i);

      if (cmd.text === undefined && cmd.callback) {
...
Listbar.prototype.appendItem = function(item, callback) {
  ...
  if (typeof item === 'object') {
    cmd = item;
    if (cmd.prefix === undefined) cmd.prefix = String(this.items.length + 1);
  }
  ...
  if (!this.disableKeysPrefix && cmd.keys && cmd.keys[0]) {
